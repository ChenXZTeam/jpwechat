...
import com.solar.tech.service.OutInfoService;
 ...
 */
@Controller
@RequestMapping("/framework/outinfo")
public class OutInfoController {
	...
	public Map<String, Object> getInfoList(int page, int rows){
		...
		map = outInfoService.getInfoList(page, rows);
		if(map.size()>0){
			map.put("msg", 1);
		}else{
			map.put("msg", 0);
	...
	@ResponseBody
	public Map<String, Object> addInfo(OutInfo outInfo,HttpServletRequest request, HttpServletResponse response, HttpSession session){
		Map<String, Object> map = new HashMap<String, Object>();
	    ...
	    int i = outInfoService.addInfo(outInfo);
	    if(i==1){
	    	map.put("msg","1");
	    }else{
	    	map.put("msg","0");
	...
	@ResponseBody
	public Map<String, Object> deleteInfo(String outinfoID) {
	     Map<String, Object> map = new HashMap<String, Object>(); 
	     try {
	    	 int i = outInfoService.deleteInfo(outinfoID);
	    	 if(i == 1){
	    		 map.put("state", 1);
				 map.put("msg", "success");
				 return map;
	...
	@ResponseBody
	public Map<String, Object> updateVisa(OutInfo outInfo){
		Map<String, Object> map = new HashMap<String, Object>();
		outInfo.setCreateTime(new Timestamp(new Date().getTime())); //创建时间赋值
		outInfoService.updateInfo(outInfo);
		map.put("state", 1);
		return map;
	}
