 ...
 * 
 * @ClassName: TaskTimer
 * @Description: 定时任务，对过期订单处理（缓存3~7天内出发的航班）
 * @author: ChenXZ
 * @date: 2016年5月6日 下午8:05:31
 */

@Component
public class TaskTimer3 {
	private static Logger log = Logger.getLogger(TaskTimer3.class);
	final long DAY_PERIOD = Long.parseLong("28800000");  //8小时
	@Autowired
	...
	public void startTask(String time) {
		...
		timer.scheduleAtFixedRate(uTask, 50000, DAY_PERIOD);
	}
	...
	private class Task extends TimerTask {
		...
		public void run() {
			...
			List<SeatInfoData> flightInfo = ptService.pastFlightMessageList("3",pastTime);
			if(flightInfo.size()>0){
				System.out.println("有3~7天内出发的航班已经过期，开始缓存");
				/*
				 *缓存步骤：
				 *1、删除之前的
				 *2、重新生成现在的
				...
				System.out.println("3~7天内的缓存航班已经缓存完成");
			}else{
				System.out.println("没有符合的过期航班，8个小时之后开始重新缓存");
			}
		}
	}
	...
	public static void main(String[] args) {
		...
		TaskTimer3 dao = ac.getBean("taskTimer", TaskTimer3.class);
		dao.startTask("订单删除");
	}
