 ...
 * 
 * @ClassName: TaskTimer
 * @Description: 定时任务，对过期订单处理（缓存15~30天内出发的航班）
 * @author: ChenXZ
 * @date: 2016年5月6日 下午8:05:31
 */

@Component
public class TaskTimer5 {
	private static Logger log = Logger.getLogger(TaskTimer5.class);
	final long DAY_PERIOD = Long.parseLong("172800000");  //2天
	@Autowired
	...
	public void startTask(String time) {
		...
		timer.scheduleAtFixedRate(uTask, 70000, DAY_PERIOD);
	}
	...
	private class Task extends TimerTask {
		...
		public void run() {
			...
			List<SeatInfoData> flightInfo = ptService.pastFlightMessageList("5",pastTime);
			if(flightInfo.size()>0){
				System.out.println("有15~30天内出发的航班已经过期，开始缓存");
				/*
				 *缓存步骤：
				 *1、删除之前的
				 *2、重新生成现在的
				...
				System.out.println("15~30天内的缓存航班已经缓存完成");
			}else{
				System.out.println("没有符合的过期航班，2天后开始重新缓存");
			}
		}
	}
	...
	public static void main(String[] args) {
		...
		TaskTimer5 dao = ac.getBean("taskTimer", TaskTimer5.class);
		dao.startTask("订单删除");
	}
