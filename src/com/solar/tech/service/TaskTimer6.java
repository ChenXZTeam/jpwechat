 ...
 * 
 * @ClassName: TaskTimer
 * @Description: 定时任务，对过期订单处理（缓存30天以上出发的航班）
 * @author: ChenXZ
 * @date: 2016年5月6日 下午8:05:31
 */

@Component
public class TaskTimer6 {
	private static Logger log = Logger.getLogger(TaskTimer6.class);
	final long DAY_PERIOD = Long.parseLong("432000000");  //5天
	@Autowired
	...
	public void startTask(String time) {
		...
		timer.scheduleAtFixedRate(uTask, 80000, DAY_PERIOD);
	}
	...
	private class Task extends TimerTask {
		...
		public void run() {
			...
			List<SeatInfoData> flightInfo = ptService.pastFlightMessageList("6",pastTime);
			if(flightInfo.size()>0){
				System.out.println("有30天以上出发的航班已经过期，开始缓存");
				/*
				 *缓存步骤：
				 *1、删除之前的
				 *2、重新生成现在的
				...
				System.out.println("30天以上的缓存航班已经缓存完成");
			}else{
				System.out.println("没有符合的过期航班，5天后开始重新缓存");
			}
		}
	}
	...
	public static void main(String[] args) {
		...
		TaskTimer6 dao = ac.getBean("taskTimer", TaskTimer6.class);
		dao.startTask("订单删除");
	}
