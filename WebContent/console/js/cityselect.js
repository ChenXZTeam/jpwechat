...
/* *
 * 格式化城市数组为对象oCity，按照a-h,i-p,q-z,hot热门城市分组：2016/7/15
    ...
    linkEvent:function(){
        ...
        for(var i=0,n=links.length;i<n;i++){
            links[i].onclick = function(){
                ...
                $("#cityName").text(this.innerHTML);
                Vcity._m.addClass('hide',that.cityBox);
                /* 点击城市名的时候隐藏myIframe */
                Vcity._m.addClass('hide',that.myIframe);
                
                //二次请求ajax
                var javaBasePath=$("#javaBasePath").text();
                alert(javaBasePath);
                $.ajax({
            	    type: "post",//数据发送的方式（post 或者 get）
            	    url: javaBasePath+"wechatController/find/dayweather.action",//要发送的后台地址
            	    data: {"val1":this.innerHTML},//要发送的数据（参数）格式为{'val1':"1","val2":"2"}
        ...
        Vcity._m.on(this.input,'blur',function(){
            ...
            if(value != ''){
                ...
                if(flag==0){
                    ...
                    $("#cityName").text(that.input.value);
                }
            }
