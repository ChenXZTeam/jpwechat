...
$(function(){
	...
	function findBySel(){
		...
		var goTimeSel = $("#goTimeSel").combobox("getValue");  
		var isOverSel = $("#isOverSel").combobox("getValue"); 
		$('#numListBox').datagrid({
		    height: '100%',
		    fit:true,
		    url: '<%=basePath%>framework/hcConfig/findBySel.action',
		    method: 'POST',
		    queryParams:{orgCity:orgCity,dstCity:dstCity,goTimeSel:goTimeSel,isOverSel:isOverSel},	 
		    striped: true,
		    nowrap: true,
		    pageSize: 10,
		    pageNumber:1, 
	...
	function saveBean(){
		 $('#lvkeInfoForm').form('submit',{
			   url: '<%=basePath%>framework/hcConfig/addHc.action',
			   onSubmit: function(){
...
<div style="padding:10px; font-size:13px;">
	<table cellspacing="10">
		<tr><td>出发城市:</td>
			...
			<td>缓存时间分类:</td>
			<td>
				<select id="goTimeSel" class="easyui-combobox" style="width:150px;" panelHeight="auto">
						<option value=""></option>
						<option value="1">当天出发</option>
						<option value="2">3天以内出发</option>
						<option value="3">3-7天出发</option>
						<option value="4">7-15天出发</option>
						<option value="5">15-30天出发</option>
						<option value="6">30天以上</option>
				</select>
			</td>
			<td>是否过期:</td>
			<td>
...
<div style="height:25px; background-color:#fff;">
	<a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true" onclick="newsBtn()">增加航班缓存</a>
